...
impl BpLoudsCommonTrait for Louds {
    fn isleaf (&self, pos:u64) -> Option<bool>{
        Option::from(self.bitString.get_bit(pos)==false)
    }
    fn parent(& self, pos:u64) -> Option<u64>{
        Option::from(self.prev_0(self.dataStruct.select_1(self.dataStruct.rank_0(pos).unwrap()).unwrap()) +1)
    }
    fn first_child(&self, pos:u64) -> Option<u64>{
        Option::from(self.child(pos, 1))
    }
    fn next_sibling(&self, pos:u64) -> Option<u64>{
        Option::from(self.dataStruct.select_0(self.dataStruct.select_1(self.dataStruct.rank_0(pos-1).unwrap()+1).unwrap()+1).unwrap()+1)
    }
}
...
mod tests {
    ...
    fn test_is_leaf(){
        ...
        assert!(test_tree.isleaf(4).unwrap());
        assert!(test_tree.isleaf(7).unwrap());
    } 
